"""
inject_sim_tracker.py — injects Simulator + Tracker JS as a separate <script> tag before </body>
"""
import sys
...
<script>
/* ================================================================
   SIMULATOR + TRACKER  — injected as standalone script (not in IIFE)
   ================================================================ */

/* ── Fallback team lists (used before data.json loads) ─────────── */
var SIM_FB = {
...
function runSimulator() {
  ...
  setTimeout(function() {
    try {
      var res = _simRun(sport, home, away, N, flags);
      _simRender(el, sport, home, away, N, res);
    } catch(e) {
      el.innerHTML = '<div class="empty">SIM ERROR: ' + e.message + '</div>';
      console.error('runSimulator', e);
    }
  }, 50);
}
...
function _simRun(sport, home, away, N, flags) {
  ...
  try {
    ...
    if (sport === 'wnba') {
      ...
      var hn = ts[home], an = ts[away];
      if (hn && an) {
        var diff = ((hn.ortg||100) - (hn.drtg||100)) - ((an.ortg||100) - (an.drtg||100));
        baseHP = 1 / (1 + Math.exp(-diff / 12));
      }
    }
    if (sport === 'tennis' && typeof tennisMatchWinProbFull === 'function') {
      ...
      var d1 = db[home], d2 = db[away];
      if (d1 && d2) baseHP = tennisMatchWinProbFull(d1, d2, 'hard', false);
    }
  } catch(e) {}

  var adjHP = baseHP;
  if (flags.home)    adjHP = Math.min(0.88, adjHP * 1.035);
  if (flags.form)    adjHP = Math.min(0.88, Math.max(0.12, adjHP + (Math.random() - 0.48) * 0.02));
  if (flags.rest)    adjHP = Math.min(0.88, Math.max(0.12, adjHP + (Math.random() - 0.48) * 0.015));
  adjHP = Math.min(0.88, Math.max(0.12, adjHP));
  ...
  var SP = { mlb:{muH:4.5,muA:4.2,sdH:1.8,sdA:1.7,ou:8.5}, nba:{muH:111,muA:108,sdH:10,sdA:10,ou:220},
             ...
             tennis:{muH:2.1,muA:1.8,sdH:0.8,sdA:0.7,ou:3.8} };
  var sp = SP[sport] || SP.mlb;
  var hWins = 0, aWins = 0;
  var hScores = [], aScores = [], totals = [], margins = [];
  ...
  for (var i = 0; i < N; i++) {
    ...
    var hAdj = adjHP > 0.5 ? (adjHP - 0.5) * sp.muH * 0.3 : 0;
    var aAdj = adjHP < 0.5 ? (0.5 - adjHP) * sp.muA * 0.3 : 0;
    var hs  = Math.max(0, sp.muH + hAdj + z1 * sp.sdH);
    var as2 = Math.max(0, sp.muA + aAdj + z2 * sp.sdA);
    if (sport === 'mlb' || sport === 'nhl') { hs = Math.round(hs); as2 = Math.round(as2); }
    else { hs = Math.round(hs * 10) / 10; as2 = Math.round(as2 * 10) / 10; }
    hScores.push(hs); aScores.push(as2);
    totals.push(hs + as2); margins.push(hs - as2);
  ...
  function dist(arr, B) {
    var mn=Math.min.apply(null,arr), mx=Math.max.apply(null,arr), bw=(mx-mn)/B||1;
    var bins=[]; for(var j=0;j<B;j++)bins.push(0);
    arr.forEach(function(v){ var bi=Math.min(B-1,Math.floor((v-mn)/bw)); bins[bi]++; });
    return {bins:bins,mn:mn,bw:bw};
  }

  var hMean=mean(hScores), aMean=mean(aScores), hStd=std(hScores), aStd=std(aScores);
  var blk = Math.floor(N/20);
  var trend = [];
  for (var b=0;b<20;b++) {
    var sl=hScores.slice(b*blk,(b+1)*blk), sl2=aScores.slice(b*blk,(b+1)*blk);
    var bw=sl.filter(function(v,i){return v>sl2[i];}).length/sl.length;
    trend.push(+bw.toFixed(3));
  }
  var outliers = hScores.map(function(h,i){return {h:h,a:aScores[i]};})
    .filter(function(g){return Math.abs(g.h-hMean)>2*hStd || Math.abs(g.a-aMean)>2*aStd;}).slice(0,6);

  return {
    N:N, hP:hWins/N, aP:aWins/N, hWins:hWins, aWins:aWins,
    hMean:hMean, aMean:aMean, hStd:hStd, aStd:aStd,
    totMean:mean(totals), totStd:std(totals),
    overP: totals.filter(function(t){return t>sp.ou;}).length/N,
    blowoutP: margins.filter(function(m){return Math.abs(m)>10;}).length/N,
    p10h:pct(hScores,10), p90h:pct(hScores,90),
    p10a:pct(aScores,10), p90a:pct(aScores,90),
    p10t:pct(totals,10),  p90t:pct(totals,90),
    hDist:dist(hScores,12), aDist:dist(aScores,12), totDist:dist(totals,12),
...
function _simRender(el, sport, home, away, N, r) {
  var p2ml = typeof window.p2ml === 'function' ? window.p2ml : function(p){ return p>=.5?-Math.round(100*p/(1-p)):'+'+Math.round(100*(1-p)/p); };
  var ml2d = typeof window.ml2d === 'function' ? window.ml2d : function(ml){ var m=parseFloat(ml); return m>0?m/100+1:100/Math.abs(m)+1; };
  var ev_  = typeof window.ev   === 'function' ? window.ev   : function(p,d){ return +(p*d-1).toFixed(4); };
  var td   = typeof window.today === 'function' ? window.today() : new Date().toISOString().slice(0,10).replace(/-/g,'');

  var hML = p2ml(r.hP), aML = p2ml(r.aP);
  var hD  = ml2d(hML),  aD  = ml2d(aML);
  var hEV = ev_(r.hP, hD), aEV = ev_(r.aP, aD);
  var favTeam = r.hP >= r.aP ? home : away;
  var favP    = Math.max(r.hP, r.aP);
  var favEV   = r.hP >= r.aP ? hEV : aEV;
  ...
  var trendMin = Math.min.apply(null,r.trend), trendMax = Math.max.apply(null,r.trend);
  var trendOK  = (trendMax-trendMin) < 0.09;

  /* Score distribution bar chart */
  function distBars(dist, col) {
    var mx = Math.max.apply(null, dist.bins) || 1;
    return dist.bins.map(function(b, i) {
      var lo = (dist.mn + i*dist.bw).toFixed(1);
      var h  = Math.round(b/mx*60)+4;
      return '<div title="'+lo+': '+((b/r.N)*100).toFixed(1)+'%" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end">'
        +'<div style="width:100%;background:'+col+';height:'+h+'px;border-radius:1px 1px 0 0;opacity:0.75"></div>'
  ...
  /* Trend bars */
  function trendBars() {
    return r.trend.map(function(v) {
      return '<div style="flex:1;background:'+(v>=0.5?'var(--nc)':'var(--hc)')+';height:'+Math.round(v*50)+'px;border-radius:1px 1px 0 0;opacity:0.7" title="'+(v*100).toFixed(1)+'%"></div>';
    }).join('');
  }
  ...
  var html =
    ...
    +'<div style="font-family:var(--mono);font-size:10px;color:var(--t3);letter-spacing:3px;margin-bottom:8px">// '+N.toLocaleString()+' SIMULATIONS &middot; '+sport.toUpperCase()+' &middot; '+away+' at '+home+'</div>'
  ...
  +'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:10px">'
    +'<div class="card"><div style="font-family:var(--mono);font-size:10px;color:var(--pc);letter-spacing:2px;margin-bottom:4px">'+home+' SCORE</div>'
      ...
      +'<div style="font-family:var(--mono);font-size:11px;color:var(--t3)">&plusmn;'+r.hStd.toFixed(1)+' '+unit+'</div>'
      +'<div style="font-family:var(--mono);font-size:11px;color:var(--t3)">P10–P90: '+r.p10h.toFixed(1)+'–'+r.p90h.toFixed(1)+'</div>'
      +'<div style="display:flex;gap:1px;height:68px;align-items:flex-end;margin-top:6px">'+distBars(r.hDist,'var(--pc)')+'</div>'
    +'</div>'
    +'<div class="card"><div style="font-family:var(--mono);font-size:10px;color:var(--gc);letter-spacing:2px;margin-bottom:4px">TOTAL</div>'
      ...
      +'<div style="font-family:var(--mono);font-size:11px;color:var(--t3)">&plusmn;'+r.totStd.toFixed(1)+' '+unit+'</div>'
      +'<div style="font-family:var(--mono);font-size:11px;color:var(--t3)">P10–P90: '+r.p10t.toFixed(1)+'–'+r.p90t.toFixed(1)+'</div>'
      +'<div style="display:flex;gap:1px;height:68px;align-items:flex-end;margin-top:6px">'+distBars(r.totDist,'var(--gc)')+'</div>'
    +'</div>'
    +'<div class="card"><div style="font-family:var(--mono);font-size:10px;color:var(--nc);letter-spacing:2px;margin-bottom:4px">'+away+' SCORE</div>'
      ...
      +'<div style="font-family:var(--mono);font-size:11px;color:var(--t3)">&plusmn;'+r.aStd.toFixed(1)+' '+unit+'</div>'
      +'<div style="font-family:var(--mono);font-size:11px;color:var(--t3)">P10–P90: '+r.p10a.toFixed(1)+'–'+r.p90a.toFixed(1)+'</div>'
      +'<div style="display:flex;gap:1px;height:68px;align-items:flex-end;margin-top:6px">'+distBars(r.aDist,'var(--nc)')+'</div>'
    +'</div>'
  +'</div>'

  /* Trend + Outliers */
  +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px">'
    +'<div class="card"><div style="font-family:var(--mono);font-size:10px;color:var(--vc);letter-spacing:2px;margin-bottom:4px">WIN PROB TREND <span style="color:'+(trendOK?'var(--nc)':'var(--hc)')+'">// '+(trendOK?'STABLE':'VOLATILE')+'</span></div>'
      +'<div style="font-family:var(--mono);font-size:11px;color:var(--t3);margin-bottom:4px">Range: '+((trendMax-trendMin)*100).toFixed(1)+'%</div>'
      +'<div style="display:flex;gap:2px;height:50px;align-items:flex-end">'+trendBars()+'</div>'
      +'<div style="display:flex;justify-content:space-between;font-family:var(--mono);font-size:9px;color:var(--t3);margin-top:2px"><span>START</span><span>END</span></div>'
      +'<div style="font-family:var(--mono);font-size:11px;color:var(--t2);margin-top:5px">Peak: <span style="color:var(--nc)">'+(trendMax*100).toFixed(1)+'%</span> &middot; Floor: <span style="color:var(--hc)">'+(trendMin*100).toFixed(1)+'%</span></div>'
    +'</div>'
    +'<div class="card"><div style="font-family:var(--mono);font-size:10px;color:var(--hc);letter-spacing:2px;margin-bottom:4px">OUTLIERS <span style="font-size:9px;color:var(--t3)">&gt;2&sigma; FROM MEAN</span></div>'
      +'<div style="font-family:var(--mono);font-size:11px;color:var(--t3);margin-bottom:4px">'+r.outliers.length+' extreme outcomes &middot; Blowout rate: '+(r.blowoutP*100).toFixed(1)+'%</div>'
      +(r.outliers.length ? r.outliers.map(function(o){
          return '<div style="display:flex;justify-content:space-between;font-family:var(--mono);font-size:12px;padding:3px 0;border-bottom:1px solid rgba(255,255,255,.04)">'
            +'<span style="color:var(--pc)">'+home+': '+(Number.isInteger(o.h)?o.h:o.h.toFixed(1))+'</span>'
            +'<span style="color:var(--nc)">'+away+': '+(Number.isInteger(o.a)?o.a:o.a.toFixed(1))+'</span>'
            +'<span style="color:'+(o.h>o.a?'var(--pc)':'var(--nc)')+'">'+( o.h>o.a?home:away)+' WIN</span>'
            +'</div>';
        }).join('') : '<div style="font-family:var(--mono);font-size:12px;color:var(--t3)">No extreme outliers — model stable</div>')
    +'</div>'
  +'</div>'
  ...
  +'<div class="card" style="margin-bottom:10px"><div style="font-family:var(--mono);font-size:10px;color:var(--gc);letter-spacing:2px;margin-bottom:8px">// ENGINE FINDINGS</div>'
    +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">'
      +'<div style="font-family:var(--mono);font-size:12px;color:var(--t2);line-height:1.9">'
        ...
        +'Win probability: <strong style="color:var(--t)">'+(favP*100).toFixed(1)+'%</strong><br>'
        +'Projected margin: <strong style="color:var(--t)">'+Math.abs(r.hMean-r.aMean).toFixed(1)+' '+unit+'</strong> &rarr; '+(r.hMean>r.aMean?home:away)+'<br>'
        +'O/U rec: <strong style="color:'+(r.overP>=.54?'var(--nc)':'var(--hc)')+'">LEAN '+(r.overP>=.54?'OVER':'UNDER')+' '+r.ou+'</strong> ('+(r.overP*100).toFixed(1)+'%)<br>'
        +'EV: <strong style="color:'+(favEV>=0?'var(--nc)':'var(--hc)')+'">'+( favEV>=0?'+':'')+( favEV*100).toFixed(1)+'%</strong> on '+favTeam+' ML'
      +'</div>'
      +'<div style="font-family:var(--mono);font-size:12px;color:var(--t2);line-height:1.9">'
        +'Score variance: <strong style="color:var(--t)">'+r.hStd.toFixed(1)+' / '+r.aStd.toFixed(1)+'</strong><br>'
        +'Blowout risk: <strong style="color:'+(r.blowoutP>.25?'var(--hc)':'var(--t3)')+'">'+( r.blowoutP*100).toFixed(1)+'%</strong><br>'
        +'Trend: <strong style="color:'+(trendOK?'var(--nc)':'var(--hc)')+'">'+( trendOK?'STABLE':'MIXED')+'</strong><br>'
        +'Sims: <strong style="color:var(--t)">'+N.toLocaleString()+'</strong><br>'
  ...
  /* Wire lock buttons */
  setTimeout(function() {
    var btnH  = document.getElementById('sim-lock-h');
    var btnA  = document.getElementById('sim-lock-a');
    var btnOU = document.getElementById('sim-lock-ou');
    var ouDir = r.overP >= .5 ? 'OVER' : 'UNDER';
    if (btnH && typeof lockPick === 'function')
      btnH.onclick  = function(){ lockPick(home, away, 'ML', home, r.hP, hML, hD, td); };
    if (btnA && typeof lockPick === 'function')
      btnA.onclick  = function(){ lockPick(home, away, 'ML', away, r.aP, aML, aD, td); };
    if (btnOU && typeof lockPick === 'function')
      btnOU.onclick = function(){ lockPick(home, away, 'OU', ouDir+' '+r.ou, r.overP, '-110', 1.91, td); };
  }, 80);
}
...
function renderLockedTracker() {
  ...
  if (!picks.length) { el.innerHTML = '<div class="empty">No locked bets yet.<br><span style="font-size:13px;color:var(--t3)">Lock picks from any game card to track them here.</span></div>'; return; }

  /* Expand team sets for re-inference */
  ...
  picks = picks.map(function(p) {
    /* Re-infer sport if stored as MLB but teams look like NHL */
    if ((!p.sport || p.sport === 'MLB') && (NHL_T.has(p.hA) || NHL_T.has(p.awA))) return Object.assign({}, p, {sport:'NHL'});
  ...
  var SPORT_COL = {MLB:'var(--hc)',NBA:'var(--pc)',NHL:'var(--ic)',TEN:'var(--mc)',WNBA:'var(--vc)',NFL:'var(--gc)',CFB:'var(--rc)'};
  var TYPE_LBL  = {ML:'MONEYLINE',RL:'RUN LINE',PL:'PUCK LINE',OU:'OVER/UNDER',SPREAD:'SPREAD',PROP:'PLAYER PROP'};
  ...
  dates.forEach(function(date) {
    ...
    byDate[date].forEach(function(p) {
      ...
      var pending  = outcome === 'pending';
      var isWin    = outcome === 'win';
      var borderC  = pending ? 'var(--nc)' : isWin ? 'var(--gc)' : 'var(--hc)';
      var prob     = Math.round((p.winProb || 0.5) * 100);
      var probCol  = prob >= 67 ? 'var(--gc)' : prob >= 55 ? 'var(--nc)' : 'var(--ic)';
      var mlStr    = p.ml ? (parseFloat(p.ml) > 0 ? '+' : '') + p.ml : '—';
      var sportTag = (p.sport || 'BET').toUpperCase();
      var sportC   = SPORT_COL[sportTag] || 'var(--t2)';
      var typeTag  = TYPE_LBL[p.betType] || p.betType || 'BET';
      var statBadge= pending
      ...
      var evStr = p.decOdds ? ((p.winProb||.5)*p.decOdds-1)*100 : null;
      var settleBtns = pending
        ...
        '<div class="gc" style="border-left:2px solid '+borderC+';margin-bottom:8px">'
          +'<div class="gch">'
            +'<div>'
              +'<div style="display:flex;align-items:center;gap:5px;flex-wrap:wrap;margin-bottom:4px">'
                +'<span style="font-family:var(--mono);font-size:10px;padding:1px 5px;border:1px solid '+sportC+';color:'+sportC+';border-radius:2px">'+sportTag+'</span>'
                +'<span style="font-family:var(--mono);font-size:10px;color:var(--t3)">'+typeTag+'</span>'
                +statBadge
              +'</div>'
              +'<div class="gtm" style="font-size:17px">'+p.betOn+'</div>'
              +'<div class="gsp">'+(p.awA||'')+(p.hA?' at '+p.hA:'')+' &middot; '+mlStr+'</div>'
            +'</div>'
          ...
          +'<div style="padding:6px 10px 8px">'
            ...
            +'<div style="font-family:var(--mono);font-size:11px;color:var(--t3)">'
              ...
              +(evStr !== null ? ' &middot; EV: '+(evStr>=0?'+':'')+evStr.toFixed(1)+'%' : '')
            +'</div>'
            +settleBtns
          +'</div>'
        +'</div>';
...
function renderParlayTracker() {
  ...
  /* Inject live PAR legs if any */
  if (typeof PAR !== 'undefined' && PAR && PAR.length >= 2) {
    var legs = PAR.map(function(l){return {betOn:l.name,ml:l.ml,prob:l.prob||.5,matchup:l.matchup,type:'ML',outcome:'pending'};});
    var combD = legs.reduce(function(a,l){return a*(typeof ml2d==='function'?ml2d(l.ml):1.91);},1);
    var combP = legs.reduce(function(a,l){return a*(l.prob||.5);},1);
    var combML= Math.round(combD>2?(combD-1)*100:-(100/(combD-1)));
    if (!parlays.find(function(p){return p.id==='cur_mlb';}))
      parlays.unshift({id:'cur_mlb',date:typeof today==='function'?today():'today',sport:'MLB',legs:legs,combinedDec:+combD.toFixed(3),combinedProb:+combP.toFixed(6),combinedML:combML,outcome:'pending',lockedAt:Date.now()});
  }
  if (typeof NBA_PAR !== 'undefined' && NBA_PAR && NBA_PAR.length >= 2) {
    var legs = NBA_PAR.map(function(l){return {betOn:l.name,ml:l.ml,prob:l.prob||.5,matchup:l.matchup,type:'ML',outcome:'pending'};});
    var combD = legs.reduce(function(a,l){return a*(typeof ml2d==='function'?ml2d(l.ml):1.91);},1);
    var combP = legs.reduce(function(a,l){return a*(l.prob||.5);},1);
    var combML= Math.round(combD>2?(combD-1)*100:-(100/(combD-1)));
    if (!parlays.find(function(p){return p.id==='cur_nba';}))
      parlays.unshift({id:'cur_nba',date:typeof today==='function'?today():'today',sport:'NBA',legs:legs,combinedDec:+combD.toFixed(3),combinedProb:+combP.toFixed(6),combinedML:combML,outcome:'pending',lockedAt:Date.now()});
  }

  if (filt !== 'all') parlays = parlays.filter(function(p){ return (p.outcome||'pending') === filt; });

  if (!parlays.length) {
    el.innerHTML = '<div class="empty">No parlays yet.<br><span style="font-size:13px;color:var(--t3)">Build a parlay using +PAR buttons on game cards, then save it here.</span></div>';
    return;
  }
  ...
  parlays.forEach(function(parl) {
    ...
    html +=
      '<div class="gc" style="border-left:2px solid '+borderC+';margin-bottom:12px">'
        +'<div class="gch">'
          +'<div>'
            +'<div style="font-family:var(--mono);font-size:10px;color:var(--t3);letter-spacing:2px;margin-bottom:3px">'+parl.legs.length+'-LEG PARLAY &middot; '+(parl.sport||'MULTI')+' &middot; '+parl.date+'</div>'
            +'<div class="gtm" style="font-size:16px;color:var(--pc)">'+combMLStr+' &middot; '+(combP*100).toFixed(2)+'%</div>'
            +'<div class="gsp">'+legsDone+'/'+parl.legs.length+' won &middot; '+legsLost+' lost &middot; '+legsPend+' pending</div>'
          +'</div>'
          +'<div style="text-align:right">'
        ...
        +'<div style="padding:0 10px 10px">'
          +'<div style="font-family:var(--mono);font-size:10px;color:var(--t3);letter-spacing:2px;margin:4px 0">LEG BREAKDOWN</div>'
          +parl.legs.map(function(leg) {
            ...
            return '<div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid rgba(255,255,255,.05)">'
              +'<div>'
                +'<span style="font-family:var(--mono);font-size:10px;padding:1px 4px;border:1px solid rgba(255,255,255,.18);border-radius:2px;color:var(--t3);margin-right:5px">'+(TYPE_LBL[leg.type]||leg.type||'BET')+(leg.sport?' &middot; '+leg.sport:'')+'</span>'
                +'<span style="font-family:var(--mono);font-size:13px;color:var(--t)">'+leg.betOn+'</span>'
              +'</div>'
          ...
          +'<div style="display:flex;justify-content:space-between;font-family:var(--mono);font-size:11px;color:var(--t3);margin-top:6px;padding-top:4px;border-top:1px solid rgba(255,255,255,.08)">'
            +'<span>Prob: <strong style="color:var(--t)">'+(combP*100).toFixed(2)+'%</strong></span>'
...
function saveParlayToTracker(legs, sport) {
  ...
  var combD = legs.reduce(function(a,l){ return a * ml2d_(l.ml || '-110'); }, 1);
  var combP = legs.reduce(function(a,l){ return a * (l.prob || .5); }, 1);
  var combML= Math.round(combD>2?(combD-1)*100:-(100/(combD-1)));
  var td    = typeof today === 'function' ? today() : new Date().toISOString().slice(0,10).replace(/-/g,'');
  parlays.unshift({id:'parl_'+Date.now(),date:td,sport:sport||'MULTI',legs:legs,combinedDec:+combD.toFixed(4),combinedProb:+combP.toFixed(6),combinedML:combML,outcome:'pending',lockedAt:Date.now()});
...
print("Injected SIM+TRACKER script tag before </body>")
